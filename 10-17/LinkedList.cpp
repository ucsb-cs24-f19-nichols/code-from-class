...
void LinkedList::clear() {
    clear(head); // the recursive helper function will do the work for us

    // at this point, the list is empty!
    // we should change head and tail now!
    head = tail = nullptr;
}
...
void LinkedList::clear(Node *n) {
    ...
    if (n == nullptr) return;
        ...
        // ^ the reason we do things in this order is because
        // we want to make sure we can still access n->next.
        // If we delete n first, n->next doesn't make sense!
        clear(n->next);
        delete n;
    }
}